...
def create_data_loaders(train_transformation,
                        ...
                        datadir,
                        args):
    traindir = os.path.join(datadir, args.train_subdir)#image/train+val/
    ...
    if args.exclude_unlabeled:
        ...
        #主索引迭代一次，次索引可以迭代多次 primary_batch_size = batch_size - secondary_batch_size
    else:
...
def rawdatawritelogger(rowdata,path):
    with open(path, 'a+', newline='') as csvfile:
            writer = csv.writer(csvfile)#默认以逗号结尾
            writer.writerow(rowdata)
            csvfile.close()

def load_args(args, isMT = False):
    label_dir = 'data-local/'
    if args.label_split < 10:
        args.labels = '%s/labels/%s/%d_balanced_labels/0%d.txt' % (\
